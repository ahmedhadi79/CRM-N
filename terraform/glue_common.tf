...
resource "aws_s3_object" "glue_salesforce_queries" {
  bucket = local.glue_assets_bucket_name
  key    = "${local.project_name}/scripts/common/salesforce_queries.py"
  source = "../src/common/salesforce_queries.py"

  etag = filemd5("../src/common/salesforce_queries.py")
}
