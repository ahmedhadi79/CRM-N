...
def cast_ambiguous_null_columns(df: pd.DataFrame) -> pd.DataFrame:
    for col in df.columns:
        if df[col].isnull().all():
            if df[col].dtype == 'object':
                df[col] = df[col].astype('string')
            elif pd.api.types.is_numeric_dtype(df[col]):
                df[col] = df[col].astype('float64')  # or another numeric fallback
...
def raw_load_to_s3(
    ...
    try:
        if file_type == "parquet":
            ingested_df = cast_ambiguous_null_columns(ingested_df)
            wr.s3.to_parquet(
                df=ingested_df,
                path=path,
...
def clean_column_values(column_name, column_values):
    """Cleans column values and attempts to convert them to the right data type."""
    column_values = (
        column_values.replace("None", None).replace("", None).dropna().astype(str)
    )

    try:
        if "date" not in column_name.lower():
            column_values = pd.Series(
                [ast.literal_eval(entry.capitalize()) for entry in column_values.values]
            )
        else:
            raise ValueError  # Skip to date parsing if column is a date
    except Exception:
        try:
            column_values = pd.Series(
...
def get_athena_dtype(column_name, column_values):
    """Determines the Athena-compatible data type for a given column."""
    try:
        if pd.api.types.is_integer_dtype(column_values):
            return "bigint" if column_values.max() > (2**31 - 1) else "int"
        elif pd.api.types.is_float_dtype(column_values):
            return "double" if str(column_values.max().dtype) == "float64" else "float"
        ...
        pass  # Fallback to string type if an error occurs

    return "string"


def get_actual_dtypes(df) -> dict:
    """Returns a schema dictionary for AWS Glue table creation."""
    result_dict = {}
    for column_name in df.columns:
        column_values = clean_column_values(column_name, df[column_name])
        result_dict[column_name] = get_athena_dtype(column_name, column_values)
